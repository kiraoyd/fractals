...
void draw_flower(double startx, double starty, double distance, int depth, double angle_degree){
...
int main(){
    ...
    double target = 360;
    int count = 1;
    while (angle_degree < target){
        sprintf(frame_name, "img%04d.bmp", count); //create a sequential framing scheme for the mpeg
        G_rgb (0.3, 0.3, 0.3) ; // dark gray
        G_rgb(0,0,0);
        G_clear () ;
        //TODO make a nice background for the flower here, maybe a gradiant?
        draw_flower(400.0, 200.0, 1.0, depth, angle_degree);
        angle_degree++;
        count++;
